...
const reference=JSON.parse(await read('source/soft-rook.json'));
const markPath=reference.path;
const [,,sourceW,sourceH]=reference.viewBox;
await write('source/mark-path.json',JSON.stringify(reference,null,2)+'\n');
...
const markH=156,markW=sourceW*markH/sourceH,wordH=96,gap=34;
const lockupW=Math.ceil(markW+gap+word.width/word.height*wordH),lockupH=172;
const lockup=fill=>mark(fill,0,8,markH/sourceH)+wordAt(fill,markW+gap,40,wordH);
for(const [suffix,color]of[['',colors.ink],['-light',colors.ivory]]){
  ...
  await write(`mark${suffix}.svg`,svg(sourceW,sourceH,mark(color)));
  await sharp(Buffer.from(logo),{density:300}).resize({width:1600}).png().toFile(path.join(root,`logo${suffix}.png`));
}
const iconScale=354/Math.max(sourceW,sourceH);
const iconMark=()=>mark(colors.ivory,(512-sourceW*iconScale)/2,(512-sourceH*iconScale)/2,iconScale);
const icon=(round=true)=>svg(512,512,`<rect width="512" height="512" rx="${round?104:0}" fill="${colors.ink}"/>`+iconMark());
...
const preview=svg(1280,760,
 ...
 label('ROOKERY / SOFT ROOK',52,42,12,colors.amberText)+label('MANROPE 700',52,297,11,colors.amberText)+
 `<g transform="translate(52 99) scale(${600/lockupW})">${lockup(colors.ink)}</g>`+
 mark(colors.ink,58,341,156/sourceH)+`<g transform="translate(289 374) scale(.16)"><rect width="512" height="512" rx="104" fill="${colors.ink}"/>${iconMark()}</g>`+
 label('Bildmarke',52,527,12,colors.ink)+label('Favicon',288,527,12,colors.ink)+
 mark(colors.ivory,892,139,202/sourceH)+wordAt(colors.ivory,804,402,94)+
 ...
 label('Ein Operator. Viele Agents.',804,663,14,colors.ivory),'Rookery: Soft Rook und Farbpalette');
...
const publicFiles=['logo.svg','logo-light.svg','mark.svg','mark-light.svg','favicon.svg','favicon.ico','apple-touch-icon.png'];
...
console.log(JSON.stringify({status:'ok',concept:reference.name,font:'Manrope',weight:700,lockup:{width:lockupW,height:lockupH},publicFiles:publicFiles.length}));
