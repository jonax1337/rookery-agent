...
/**
 * The same clip budget the Claude Code adapter puts on a tool result, mirrored
 * from core's `TurnBlocks`: a transcript that survives a reload is worth more
 * than a verbatim one nobody can afford to store.
 */
 ...
 *
 * Web-side mirror of `packages/core/src/util/blocks.ts` (core is a Node
 * package and must stay out of the browser bundle) - the state machine is the
 * same one, including the id match and the newest-open fallback, so the local
 * placeholder renders exactly what the persisted `blocks` will show after a
 * reload. One instance lives for the whole turn.
 */
export class TurnBlocks {
   ...
   * and no tool ran after it. Anything else keeps the deltas, and the flat
   * `content` stays authoritative for consumers that never look at `blocks`.
   *
