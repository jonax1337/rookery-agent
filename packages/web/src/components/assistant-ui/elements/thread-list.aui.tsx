...
const dateGroupLabel = (
  ...
  if (!date || date.getTime() >= startOfToday) return "Heute";
  if (date.getTime() >= startOfToday - DAY_IN_MS) return "Gestern";
  return "Früher";
};
  ...
  return useMemo(() => {
    ...
    const filteredIndices = threadIds
      ...
      .filter(
        ({ id }) =>
          !query ||
          (itemsById.get(id)?.title || "Neues Gespräch")
            .toLowerCase()
            .includes(query),
      )
      .map(({ index }) => index);
    ...
    <div className="flex flex-col gap-0.5">
      {Array.from({ length: 5 }, (_, i) => (
        <div
          key={i}
          role="status"
          aria-label="Gespräche werden geladen"
    ...
    >
      {isRenaming ? (
        ...
        >
          ...
          >
            <ThreadListItemPrimitive.Title fallback="Neues Gespräch" />
          </span>
          {isRunning && <span className="sr-only">Läuft</span>}
        </ThreadListItemPrimitive.Trigger>
      )}
