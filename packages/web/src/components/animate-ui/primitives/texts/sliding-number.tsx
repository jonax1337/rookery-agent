...
function SlidingNumber({
  ...
  // What a screen reader should hear: the settled number, once. The rollers
  // below stack all ten digits per place and translate the column - without
  // this, the accessible name of every count reads "0 1 2 … 9".
  const srNumber = formatNumber(Math.abs(Number(number)));
  const [srInt = '', srDec = ''] = srNumber.split('.');
  const srIntGrouped =
    typeof thousandSeparator === 'undefined'
      ? srInt
      : srInt.replace(/\B(?=(\d{3})+(?!\d))/g, thousandSeparator);
  const srText =
    (Number(number) < 0 ? '-' : '') + srIntGrouped + (srDec ? decimalSeparator + srDec : '');
